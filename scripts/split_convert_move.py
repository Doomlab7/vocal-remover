...
if __name__ == "__main__":
    for file in Path("/app/downloads/raw").glob("*.mp4"):
        ...
        convert_mp4(data)
    for file in Path("/app/downloads/raw").glob("*_Instruments.wav"):
        data = InferenceRequest(filename=str(file.with_suffix("")))
        # convert _instruments.wav to _instruments.mp3
        convert_wav(data)
    # move .mp3s to to_upload
    for move_these in Path("/app/downloads/raw").glob("*.mp3"):
        move_these.rename(Path("/app/downloads/mp3s/to-upload") / move_these.name)
